    ...
    class Usuario extends Conectar{
        ...
        public function login(){
            ...
            if(isset($_POST["enviar"])){
                ...
                if(empty($correo) or empty($pass)){
                    /* En caso esten vacios correo y/o contraseña, devolver al index con mensaje = 2 */
        ...
        public function get_cursos_x_usuario_top10($usu_id){
            ...
            $sql="SELECT 
                ...
                td_curso_usuario.usu_id = ?
                AND td_curso_usuario.est = 1
                ORDER BY tm_curso.cur_fechfin DESC
                LIMIT 10";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $usu_id);
            $sql->execute();
        ...
        public function insert_usuario($usu_nom,$usu_apep,$usu_apem,$usu_correo,$usu_pass,$usu_sex,$usu_telf,$rol_id,$usu_dni){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="INSERT INTO tm_usuario (usu_nom,usu_apep,usu_apem,usu_correo,usu_pass,usu_sex,usu_telf,rol_id,usu_dni,fech_crea,est) VALUES (?,?,?,?,?,?,?,?,?,NOW(),1);";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $usu_nom);
            $sql->bindValue(2, $usu_apep);
            $sql->bindValue(3, $usu_apem);
